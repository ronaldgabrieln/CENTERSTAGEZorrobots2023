...
public class ZorroAutoREDBACKDROP extends LinearOpMode {
    //Garras
    ...
    Trajectory adjustRightBackdrop;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        backdropRight = drive.trajectoryBuilder(twoInchAdvancementToSpikeMark.end().plus(new Pose2d(0,0,Math.toRadians(180))),false )
                .back(36.5)
                .build();
        moveBackdropToLeft = drive.trajectoryBuilder(backdropRight.end())
                .strafeLeft(10)
                .build();
        adjustLeftBackdrop = drive.trajectoryBuilder(moveBackdropToLeft.end())
                .back(7)
                .build();
        adjustRightBackdrop = drive.trajectoryBuilder(backdropLeftAdvancement.end())
                .strafeLeft(25)
                .build();
        parkRight = drive.trajectoryBuilder(moveBackdropToLeft.end())
                .strafeLeft(37)
                .build();
        parkCenter = drive.trajectoryBuilder(backdropCenter.end())
                .strafeLeft(35)
                .build();
        parkLeft = drive.trajectoryBuilder(adjustRightBackdrop.end())
                .strafeLeft(15)
                .build();
        //Elevador
        ...
        while(opModeIsActive() && !isStopRequested()) {
            telemetry.addData("Brazo: ", Brazo.getCurrentPosition());
                if (direccion == 0) {
                    ...
                    Muneca.setPosition(.9);
                    sleep(200);
                    drive.followTrajectory(adjustRightBackdrop);
                    GarraIzq.setPosition(1);
                    sleep(1500);
                    Muneca.setPosition(0);
